...
const ACT =
  nextEp <= 20 ? "1막 (회귀·회피)" :
  nextEp <= 45 ? "2막 (음모 파악·가문 지키기)" :
  nextEp <= 65 ? "3막 (반격·반전)" :
  nextEp <= 85 ? "4막 (진실·대결 구도)" : "5막 (단죄·해방)";
const PACING =
...
async function fetchSteering() {
  ...
  try {
    const res = await fetch(`${SUPABASE_URL}/rest/v1/novel_steering?status=eq.pending&novel_id=eq.romance&order=created_at.asc&select=id,note`,
...
// ── 정통 문법 ─────────────────────────────────────────────────
const STYLE = `# 회귀 로맨스 판타지 문법 — 재미가 1순위 (반드시 준수)
- **첫 3줄 후킹**: 풍경·분위기 묘사로 천천히 시작 금지. 감정·위기·반전·대사로 즉시 끌어당겨라.
- **사이다 ≥ 1**: 매 화 반드시, [[세실리아]]가 전생 기억으로 배신자·집착남의 수를 *미리 읽고* 받아치거나 한 수 위에서 노는 통쾌함(단, 티 안 나게 우아하게). 답답함(고구마) 길게 끌기 금지.
- **절단신공**: 마지막 1~2문장은 강한 훅/반전/감정의 일격으로 끊어 다음 화를 못 참게.
- **로판 결**: 사교계·관계·감정선의 긴장을 매 화 한 스푼(시선·말 한마디·소문·예법). 신파·자기연민·질질 끌기 금지.
- **문체**: 우아하되 빠른 문장. 대사·내적 독백 활용. 과한 미사여구 금지, 가독성 우선.
- **차별점 유지(THE HOOK)**: premise(한 끗)를 매 화 살려라 — *회피가 무기, 집착은 피할수록 강해진다.* 착하고 수동적으로 당하는 회귀 영애 클리셰로 회귀 금지.
- 분량: 본문 2500~3500자. 5분 안에 읽히게.
- 시점: [[세실리아]] 밀착(1인칭 또는 3인칭 제한).
- 연속성 최우선: 캐논(프리미스·전생기록·세계관·인물)과 **한 줄도 모순 금지**. 전생 사건·인물 정체는 캐논 그대로.
...
description: 집착광에게 사랑이라 속아 처형당한 영애가 운명의 무도회 날로 회귀했다. 가문과 사람들을 지키고 그 남자의 음모를 피하며 해체한다. 매일 아침 한 화씩 자동으로 이어지고, 독자가 방향을 던지면 그대로 반영됩니다. (시즌1 전 ${TARGET}화)
stats:
